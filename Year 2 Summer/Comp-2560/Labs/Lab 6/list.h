...
list *List(){
    ...
    newList -> list = (allTypes*) malloc(sizeof(allTypes));
    newList -> len = 0;
    newList -> CurrentSize = 1;
    grow(newList);
    return newList;
}
...
void fillElement(allTypes *temp, type_t type, void *appendList){
    temp -> type = type;
    int num = *(int*) appendList;
    switch(type){
        case INT:{
            if ()
            temp -> element = (void*) calloc(1, sizeof(int)); break;
        }
        case FLOAT:     temp -> element = (void*) calloc(1, sizeof(float)); break;
        case DOUBLE:    temp -> element = (void*) calloc(1, sizeof(double)); break;
        case LDOUBLE:   temp -> element = (void*) calloc(1, sizeof(long double)); break;
        case CHAR:      temp -> element = (void*) calloc(1, sizeof(char)); break;
        case CHARp:     temp -> element = (void*) calloc((strlen((char*) appendList) + 1), sizeof(char)); break;
        case LIST:      break;
        case BOOL:{
            #ifdef _STDBOOL_H
            temp -> element = (void*) calloc(1, sizeof(bool)); break;
            #else
    ...
    if (type == CHARp) for (int i = 0; ((char*) appendList)[i] != '\0'; i++) ((char*) temp -> element)[i] = ((char*) appendList)[i];
    else temp -> element = appendList;
}
