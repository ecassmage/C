...
#else
    printf("\033[0;31m");
    printf("\nWe Caught an ERROR message in \"str.h\" but \"ERROR.h\" was not included so this message played.\n");
    printf("\033[0m");
    exit(253);
#endif
...
void removeNCharacter(str *String, unsigned index);
/// Deluxe <string.h> Rip Off START
...
void freeStringOnly(str *String);
...
void concatSingleStr(str *String, str *arr){
    concatSingle(String, toCharArray(arr));
}
...
void removeNCharacter(str *String, unsigned index){
    if (index >= len(String)) errorString(0, 3, index, len(String), "void removeNCharacter(str *string, unsigned index);");
    for (unsigned i = index; i < len(String); i++){
        if (i+1 == len(String)) String -> string[i] = '\0';
        else    String -> string[i] = String -> string[i+1];
    }
    String -> len--;
}
/// Cleaning Protocols END
